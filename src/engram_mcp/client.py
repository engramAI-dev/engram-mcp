...
The plugin does not touch the DB directly — it calls authenticated
HTTP endpoints on the Engram backend. This keeps business logic
centralized and means the plugin install story is "paste your Engram
token into the config", not "set up a local Postgres with pgvector".
"""
...
class EngramClient:
    """Implements the `engram_mcp.tools.Dispatcher` Protocol."""
